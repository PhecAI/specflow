...
- [ ] 若为 dispatch_array（**自动托管下的 per-group 快照派发**）：数组内每个元素带 `groupId` / `agent` / 独立 `focusPlan` / `dependsOn`（引擎解析 plan.md Group 标题中的"依赖 Group X"声明）；**Cursor 执行规则**：① item.dependsOn 为空 → **同一消息内并行**发起多个 `Task` tool call；② item.dependsOn 非空 → 按依赖顺序**串行**调用 Task（被依赖方先完成）；按 `waitPolicy=any_done`：任一 Task 返回即先跑一轮引擎，仅推进该 Group 的下一步。**每个 Group 的 implement→QA→fix→QA 闭环独立运行**
...
- **自动托管下的并行派发（per-group 快照）**：引擎在每一轮根据所有未完成 Group 的**当前快照**产出 `dispatch_array`——按 Group 状态分别派 `specflow-implement`（含 Bug Fix 模式）或 `specflow-qa`，同一批内**可以混合**（例如 A 已经 `[?]` 派 `specflow-qa`，B 还在 `[ ]` 派 `specflow-implement`）。编排层按 `waitPolicy=any_done` 执行：谁先返回先推进谁，不等待其余 Group；每次推进后立刻再跑引擎生成该 Group 的下一步。
...
- **Cursor Agent 并行/串行执行规则**：`dispatch_array` ≠ 自动并发——编排层**必须**主动判断：
  - items 中所有 `dependsOn` 均为空 → 在**同一消息**内并行发起多个 `Task` tool call（每 item 对应一个 Task）
  - 任一 item 含非空 `dependsOn` → 按依赖顺序**串行**调用 Task，依赖方先完成再启动被依赖方
...
## 归档锚点（状态：`archiveAnchorDone`）

- **Roadmap 全绿 ≠ 可归档**：测试期间仍可能产生需求变更，过早合并知识库/代码规范会把"未冻结"的产物灌入全局资产。因此引擎 **不自动开始合并，也不弹 AskQuestion**。
- **引擎侧**：当所有 Group 完成、`archiveAnchorDone !== true` 时返回 `anchor`（`next.action === 'set-archive-anchor'`），并在 `message` 里说明「请在确认测试通过、需求不再变更时主动告知归档」。
- **编排侧**：
...
- **锚点落地后**：引擎依次 `dispatch`：`specflow-domain-explorer`（Merge）→ `specflow-knowledge-reviewer` → `specflow-archive`；全局资产合并由 `specflow-archive` 统一执行，任何 merge 步骤都**不会**先于 `set-archive-anchor` 发生。
...
- “dispatch_array 出来了，我挨个串行做完再跑引擎就行”
...
| “dispatch_array 我串行做就行”      | 无 dependsOn 的 Group 应并行 Task call；串行会拖慢整体进度      |
