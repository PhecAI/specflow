...
- **允许**：为业务项目建 `ai-docs/global-assets/` 空壳——空 `domains/index.md`（只有表头）、空 `metadata.json`、默认 `standards/code-style.md` 与 `standards/architecture-layers.md` 模板（不覆盖既有文件）。
- **禁止**：扫描 `src/services` / `src/modules` / `src/domains` 或任何固定目录来"猜"领域；禁止把目录名当成领域名；禁止在 `init` 阶段生成任何 `<domain>.md`。
- **领域识别与命名**：一律交给 `specflow-domain-explorer`（Recommend / Explore 模式）在独立 agent 轮次中完成。该 agent 会读 README / 代码语义 / 既有全局领域库，产出带证据的领域身份（`<scope>::<slug>`），然后通过 `inventory-scan.cjs add-domain` 原语落盘。
...
5. 若 `standards/architecture-layers.md` 不存在，按默认模板写入；此处只建协议骨架，不在 init 阶段硬编码前端/后端分层。
...
- 当需求启动或后续需要为项目补录领域时，由 Orchestrator 派发 `specflow-domain-explorer`（Recommend 模式），由其产出语义明确的领域身份（`<scope>::<slug>`）+ 证据路径，然后调用：
