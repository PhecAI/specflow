...
- focusSpecify 由引擎从 specify.md 提取，含 Executive Summary、Entity Inventory、Business State Map、Non-Functional；不含已闭合 Clarification Log 与 Changelog。
...
- **契约依据（强制）**：若用户未在规格中提供可落地的 **接口/字段变更** 依据，子代理 **不得** 在 `plan.md` 中编造具体接口路径与字段；**必须**按 `agents/specflow-plan.md` Phase 0 在 `specify.md` 插入未闭合 `[?]` 打回，待用户闭合后再生成 plan。
