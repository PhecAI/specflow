...
### 场景：阻塞性澄清未闭合强行继续
- **特征**：Specify 阶段 Clarification Log 中存在**未闭合的阻塞性** `[?]`，但用户未回答直接要求进入 Plan。
- **Action**：**拒绝**，引用 `specflow-specify.md`：阻塞性缺口必须由用户闭合；引擎交互每轮最多展示 3 条 CQ，其余请在 `specify.md` 中逐项填写 **[User]**。
- **话术模板**：
  > ✋ **流程暂停**
  > 存在未闭合的阻塞性澄清（`[?]`）。请先在 Clarification Log 中为每条在其下方 #### **[User]** 处填写决策（或通过交互工具选择），全部闭合后再进入 Plan。若为「为问而问」的非阻塞项，应删去 `[?]` 并在正文写入默认假设。
